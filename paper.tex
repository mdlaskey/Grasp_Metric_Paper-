...
Recent work has studied the effects of uncertainty in object pose and gripper positioning.%, motivated by part tolerancing from manufacturing and the use of low-cost sensors such as the Kinect.
...
To address the computational cost, Kehoe et al.~\cite{kehoe2012estimating} proposed an adaptive sampling procedure known as Iterative Pruning, which at a set iteration discards a subset of the grasps that seem unlikely to be of high quality . However, the method pruned grasps using only the sample mean and did not utilize variance around the current estimate, which can discard good grasps.
...
Since then several other algorithms have been shown to achieve near this bound in terms of convergence, such as the Gittins index policy~\cite{weber1992gittins} and Thompson sampling for the two arm case\cite{agrawal2011analysis}. \todo{The last result is actually only a few years ago. I can talk about contextual bandits for the sake of other recent information, but we don't use them and we are short on space. So I am wondering your opinion here}
...
\begin{figure}[ht!]
\centering
\includegraphics[width = 9cm, height = 9cm]{matlab_figures/confidence_bounds.eps}
\caption{ \footnotesize \todo{We might want to condense Fig 6 and Fig. 4. The confidence intervals clutter the plot though} The probability of Force Closure of the grasp returned vs the number of samples required with a $95\%$ confidence interval around the return value for Thompson Sampling, the adaptive sampling method and Monte-Carlo sampling. Graph is averaged over 100 objects from the Brown Silhouette Dataset \cite{brown} with a set $|G|=1000$ for each object. The sharper decrease in confidence interval corresponds to more samples being allocated for the return grasp. }
