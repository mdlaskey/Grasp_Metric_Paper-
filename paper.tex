...
\author{Michael Laskey$^1$, Zoe McCarthy$^1$, Jeff Mahler$^1$, Florian T. Pokorny$^3$, Sachin Patil$^1$,\\ Jur Van Den Berg$^4$,  Danica Kragic$^3$, Pieter Abbeel$^1$, Ken Goldberg$^2$% <-this % stops a space
...
Traditional bandit algorithms minimize cumulative regret, which is the sum over each sub-optimal arm of the number of times that arm is pulled times the difference between the true expected reward of an optimal arm and the true expected reward of that sub-optimal arm.
...
Different to the normal bandit problem, we do not actually care about cumulative regret because the exploration and exploitation stage is decoupled from each other. For example in medical testing for patients it is important to always administer the best treatment for the given patient. However for the case of product testing in cosmetics it does not matter what products are administer in the testing phase, but only what final product is sold to the consumer.
...
for the analysis of the existing bandit algorithms like UCB1.
...
Since we are in an uncertain environment, we are interested in calculating the expected quality, or $P(Q>0)$, or the probability of achieving force closure, similar too  \cite{christopoulos2007handling}\cite{kehoe2012toward} . $P(Q>0)$ is computed by sampling from our distributions on pose, shape and material properties (friction coefficient and center of mass) and averaging the qualities that are computed. Reducing the time to find the best expected grasp from a set of a large number of grasps is the primary focus of the paper. 

\subsection{Types of Uncertainty}
In this work we consider the following types of uncertainty shape, pose, center of mass and friction coefficient. The probabilistic relationship of all these types of uncertainty can be seen in Fig. \ref{fig:graphical_model}. We model each one of these with a corresponding distribution and demonstrate how MAB algorithms can handle all types of uncertainty.

For shape, we use a GPIS representation as described in Sec. \ref{sec:GP}. For pose we sample the centroid of the GPIS from a Gaussian distribution with user covariance matrix. Center of mass a Gaussian  with mean as the expected center of mass, as defined in Eq. \ref{eq:mass}, assuming uniform mass distribution. Lastly, for friction coefficient we represent it as Gaussian with mean around the expected friction coefficient provided by a user. 
...
While we currently assume the gripper moves free of noise, this approach would be applicable to high precision robots such as industrial robots. 
...
\Gamma^* \in \mbox{argmax}_{\Gamma \in G} P(Q(\Gamma)>0)
\end{align}
...
In solving Eq. \ref{eq:problem_def}, we want to pick the grasp plan with the highest $P(Q(\Gamma)>0)$ determined via Monte-Carlo Integration.
Since the function to test force closure is boolean, we can model our problem as choosing the best arm for a Beta-Bernoulli Process. Methods with very strong guarantees are now available to us \cite{review}. We now review some of them.
\section{Bandit Algorithms For Best Arm Identification}
...
The UCB strategy was the first strategy shown to have asymptotic logarithmic regret \cite{lai1985asymptotically} for distributions with bounded rewards. It basically assumes a frequenist perspective and estimates an empirical confidence bound for each arm based on the observations seen so far. Then it pulls the arm with the highest confidence bound. 


\subsection{Gittins Index} 
Gittins index treats the bandit problem as an Markov Decision Process where each lever pull is taking a step along a Markov Chain. While, normally the cost of computing an optimal policy would be exponential in the number of K arms. The independence assumption between each arms allows us to decouple them and solve for a policy that models taking pulls along one arm or selecting a reference arm that represents selecting a different possible value. We computed the Gittins indices using the restart method proposed by Katehakis et al. \cite{katehakis1987multi}.
...
Here $\alpha_0$ and $\beta_0$ correspond to the intial shape parameters. To compute the above distribution we must draw samples from $p(S)$. In order to draw shape samples from a GPIS, one needs to sample from signed distance function, $sd$, over the joint on all points in the workspace $\mathcal{W}$ or $p(sd(\mathcal{W}))$. Since this is a GPIS, we know the following 
