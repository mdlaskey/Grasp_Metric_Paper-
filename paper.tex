...
%%PA: continuous and differentiable (which I believe would be a redundant statement as differentiable implies continuous), or continuously differentiable? %%ZM: fixed
...
More formally an agent is given a set prior distributions $\mathcal{P}(1,...,K)$ for the K arms based on the rewards seen. At each interval an arm $I_t$ is pulled based on some allocation strategy $\psi_t \in \lbrace 1,...,K \rbrace$. This happens until a stopping signal is given at which point an arm (or grasp plan) is chosen based on a recommendation strategy $\zeta_t \in \lbrace 1,...,K \rbrace$. 
