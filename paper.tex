...
\title{\LARGE \bf
Multi-Arm Bandit Models for 2D Grasp Planning with Uncertainty  [v11 March 13, 2015 ] }
...
\begin{figure}%
    ...
    \caption{\todo{figure is only averaged over 40 shapes, rerunning the experiments (Shouldn't change earlier results on (TS, IP and UA ) once done)} Grasp quality over time for 4 sample-based planning methods compared with optimal grasp determined by brute force search over all candidate grasps (which required 10x  more time than any of these methods \cite{kehoe2012toward}).  Data is based on 100 objects randomly selected from from the Brown Vision 2D Lab Dataset \cite{brown} with 1000 uniformly distributed candidate grasp angles. Monte Carlo methods such as Uniform sampling and iterative pruning (eliminating candidates that perform poorly on initial samples) eventually converge to within 3$\%$ of the optimal, requiring approximately 40,000 and 20,000 iterations.  Two Multi-Arm Bandit methods perform significantly better, converging after only 4000 and 2000 iterations. }%
...
Our initial results in simulation show that the Gittins Index, a MAB algorithm, required $808\%$ fewer samples than Iterative Pruning to plan a grasp within $3\%$ of the estimated highest probability of force closure in the set of 1000 randomly selected grasps averaged over 100 objects.
...
\todo{Ken, you mentioned this section might seem condescending to the OR community. It be great if you can point out specifics here}
...
Computation of the Gittins indices can increase exponentially in time as the discount factor approaches $1$. The problem with this is that the effective time horizon is a function of $\gamma$. Thus, if your actual horizon is longer than the effective horizon, the method has a policy of Follow the Leader, where it only pulls the current estimate of the best arm \cite{kelly1981multi}. Thompson sampling is an alternative to the GIttins Index method that isn't prone to such a problem. 
...
\todo{Ken, the next section was crossed out in your notes, I'm still not clear on whats wrong with it. Can you elaborate more?}
Given a grasp plan and a object, we define the {\it contact points} as the spatial locations at which the jaws come into contact with the object when following along the grasp axis, $\bc_1, \bc_2 \in \mathbb{R}^2$.
...
\todo{This might still be vague} In Fig. \ref{fig:grasp_quality}, we plot the grasp returned at time $t$ vs. $P(\Gamma_{\bar{k}})$, the probability of force closure for the grasp returned by the algorithm $\Gamma_(\bar{k})$.  Monte Carlo methods such as Uniform sampling and iterative pruning (eliminating candidates that perform poorly on initial samples) eventually converge to within 3$\%$ of the optimal, requiring approximately 40,000 and 20,000 iterations.  Gittins Indices and Thompson Sampling  perform significantly better, converging after only 4000 and 2000 iterations.For illustrative purposes in Fig. \ref{fig:shape_samples}, we select a stopping time $T_s=10,000$, which is 10 samples per grasp on average, and for each method visualize the grasp plan returned, $\Gamma_{\bar{k}}$, on 3 randomly selected shapes in the dataset.
...
We also compared the methods by the number of samples each allocated to the top $10\%$ of grasps in terms of the probability of force closure estimated by brute force.
...
Uniform allocation and Iterative Pruning allocate approximately $1\%$  and $6\%$ of total samples, respectively. The MAB appear to allocate more samples to higher quality grasps, which may explain the ability to find grasps with a higher probability of force closure faster.
...
\todo{need to redo with larger number of shapes (Running experiments today)}
...
The question about what MAB algorithm to use, Thompson Sampling or Gittins Indices. Is left to the application the user is considering. Empirically, we showed Gittins indices are able to return a grasp that is within $3\%$ of the best grasp 2x faster than Thompson Sampling. However because we aren't able to compute a higher discount factor than $\gamma = 0.98$, the method is prone to only pulling the best arm \cite{kelly1981multi}. Thompson sampling avoids this problem and is able to find the best grasp in the set \cite{agrawal2011analysis}, but empirically was shown to require more samples than the Gittins to reach $3\%$ of the best grasp.
