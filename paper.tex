...
To address the computational cost, Kehoe et al.~\cite{kehoe2012estimating} proposed a parallelizable adaptive sampling procedure called iterative pruning, which periodically discards a subset of the grasps that seem unlikely to be of high probability of force closure. However, the method pruned grasps using only the sample mean, which could discard good grasps.
We propose modeling the problem as a Multi-Armed Bandit, which selects the next grasp to sample based on past observations ~\cite{barto1998reinforcement, lai1985asymptotically}. The MAB method is still parallizable if you divide up the divide up the arms into sub-problems \cite{glazebrook2000index}
...
A traditional MAB example is a gambler with $K$ independent slot machines, also known as one-armed bandits.
...
The goal of the gambler is to maximize the sum of average rewards over all pulls.
...
Computation of the Gittins indices can increase exponentially in time as the discount factor approaches $1$. In the case of finding the best arm, we want to plan for long term reward and thus have a high discount factor. A smaller $\gamma$ can lead to the policy only pulling the most promising arm and not exploring \cite{kelly1981multi}.Thompson Sampling is an alternative to the Gittins Index method that isn't prone to such a problem. 
...
\caption{A graphical model that illustrates the relationship between the different types of uncertainty in an object. As illustrated uncertainty in pose, motion and shape affect the contacts and surface normals that makes up the grasp. Friction coefficient is independent of this relationship.  }
\vspace*{-10pt}
...
In future work, we plan to scale our method to 3D objects. However, this could substantially increase the number of candidate grasps. Glazebrook and Wilkinson showed that the Gittins Index method could be parallelized by simply pulling the top $M$ arms, where $M$ is the number of cores and achieve better performance \cite{glazebrook2000index}. A similar method could also be done for Thompson sampling. Another promising scheme for parrallelizing the MAB, is to divide up the MAB into batches of arms have each core solve the smaller MAB. We will explore both of these approaches in future work. 
