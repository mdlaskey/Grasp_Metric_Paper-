...
\begin{abstract}
Uncertainty arising from noise, occlusions, and surface properties such as transparency and specularities can make it difficult to plan grasps.
A popular approach to handle uncertainty is to sample perturbations in shape, state, and control to calculate the probability of force closure.
Common methods uniformly allocate samples to rank a set of potential grasp, but this may require a large number of samples to converge, even for planar models.
...
Our initial results in simulation show that Thompson Sampling, a MAB algorithm, required $439\%$ fewer samples than Iterative Pruning to plan a grasp within $2.5\%$ of the estimated highest probability of force closure in the set of 1000 randomly selected grasps averaged over 100 objects.
\end{abstract}
...
One option is using Monte-Carlo integration over sample perturbations in the uncertain quantities to evaluate the probability of force closure for a set of candidate grasps~\cite{christopoulos2007handling, kim2012physically, weisz2012pose, kehoe2012toward}, but this can be computationally expensive. 

We show that by using a Multi-Armed Bandit (MAB) model it is possible to allocate sampling effort to only grasps that seem promising. The MAB model for sequential decision making~\cite{barto1998reinforcement, lai1985asymptotically, robbins1985some} provides a formal way to reason about allocating sampling effort. In a standard MAB there are a set of possible options, or `arms'~\cite{barto1998reinforcement}, that each return a numeric reward from a stationary distribution. The goal in a MAB problem is to  select a sequence of options to maximize expected reward. MAB algorithms can be interpreted as anytime algorithms, where at a provided stopping time the algorithm terminates and returns the estimated best grasp, with respect to a chosen metric, or continues running until a user defined confidence level is met. 
...
Furthermore, we model uncertainty in motion as a normal distribution around the center and angle of the grasp axis for a parallel jaw gripper and uncertainty in friction coefficient as a normal distribution around an expected friction coefficient.

We compare the performance of Thompson sampling and Gittins indices, two popular algorithms for solving the MAB problem against uniform allocation and an adaptive sampling method known as Iterative Pruning, which reduces the set size based on sample mean~\cite{kehoe2012toward}, on the task of finding grasps with high probability of force closure.
We compare on the Brown Vision 2D Dataset, a dataset of 2D object silhouettes~\cite{brown, christopoulos2007handling}.
Our initial results show that Thompson Sampling, a MAB algorithm, required $439\%$ fewer samples than Iterative Pruning to get within $2.5\%$ of the estimated highest probability of force closure in the set of 1000 randomly selected grasps averaged over 100 objects.
...
Kim et al.~\cite{kim2012physically} planned grasps using dynamic simulations over perturbations in object pose, and also found that the planned grasps were more successful on a physical robot than thoise planned with classical wrench space metrics.
...
To address the computational cost, Kehoe et al.~\cite{kehoe2012estimating} proposed an adaptive sampling procedure known as Iterative Pruning, which periodically discards a subset of the grasps that seem unlikely to be of high quality. However, the method pruned grasps using only the sample mean and did not utilize any confidence interval around the current estimate, which can discard good grasps.
...
The goal of the gambler is to come up with a method for determining which arms to pull, how many times to pull each arm, and what order to pull them to maximize the sum of average rewards over all pulls.
...
We consider Bayesian MAB algorithms that use previous samples to form a belief distribution on the likelihood of the parameters specifying the distribution of each arm \cite{weber1992gittins,agrawal2011analysis}, as these methods have been shown empirically to outperform frequentist algorithms (e.g., UCB) on certain problems~\cite{chapelle2011empirical, bachman2013greedy}. Theoretical results have shown that several algorithms are capable of achieving near the lower bound on the asymptotic rate of convergence in regret described by Lai and Robbins~\cite{gittins1983dynamic, agrawal2011analysis, kaufmann2012bayesian}.
...
Given the current belief $\alpha_{k, t}, \beta_{k, t}$ for an arm $k$ at time $t$, the algorithm can predict the probability of an event occurring, $p_{k,t}$, on the next iteration by taking the expected value:
...
One method to approximately solve Equation~\ref{eq:old_problem_def} is to exhaustively evaluate $P_F(\Gamma_k)$ for all grasp plans in $\mG$ using Monte-Carlo integration and then sort the plans by this quality metric.
...
\end{align}
\noindent where the $\bar{k}$ grasp plan refers to the grasp with the highest estimated $P_F$ at time $t$. 

\section{Simulation Experiments}
We used the Brown Vision Lab 2D dataset \cite{brown}, a database of 2D object silhouettes, as in \cite{christopoulos2007handling}.
...
The measurement noises were assigned uniformly at random to the grid cells.
...
We drew random grasp plans $\Gamma$ by sampling the angle of grasp axis around a circle with radius $\sqrt{2} M$, where $M$ is the dimension of the workspace, and then sampling the circle's origin. 
...
\todo{This might still be vague} In Fig. \ref{fig:grasp_quality}, we plot the grasp returned at time $t$ vs. $P(\Gamma_{\bar{k}})$, the probability of force closure for the grasp returned by the algorithm $\Gamma_(\bar{k})$.  As illustrated, Gittins and Thompson return a grasp plan that on average has a higher probability of force closure than uniform allocation and Iterative Pruning . For illustrative purposes in Fig. \ref{fig:shape_samples}, we select a stopping time $T_s=10,000$, which is 10 samples per grasp on average, and for each method visualize the grasp plan returned, $\Gamma_{\bar{k}}$, on 3 randomly selected shapes in the dataset.

The MAB algorithm can also be terminated when the $95\%$ confidence interval around the returned grasp (see Equation \ref{eq:confidence}) is below a set threshold $\epsilon$. We plot the confidence intervals around the returned grasp vs. the number of samples drawn in Fig. \ref{fig:confidence} for Thompson Sampling, Iterative Pruning\cite{kehoe2012toward} and uniform allocation. \todo{How should we quantify this? Uniform allocation never gets a low confidence over the number of samples we used.} As illustrated, the confidence interval for Thompson sampling converges at a faster rate than the other two methods. 

We also compared the methods by the number of samples each allocated to the top $10\%$ of grasps in terms of the pprobabilty of force closure estimated by brute force.
We found that Thompson Sampling allocates 29$\%$ of the total number of samples to the top $10\%$ of grasps and Gittins allocates 23$\%$ of total samples.
Uniform allocation and Iterative Pruning allocate approximately $1\%$  and $6\%$ of total samples, respectively.
In Fig. \ref{fig:pulls_per_grasp} we plot samples per grasp quality.
The MAB appear to allocate more samples to higher quality grasps, which may explain the ability to find grasps with a higher probability of force closure faster.
...
All experiments were averaged across 10 shapes randomly selected with from the Brown dataset with $|\mG| = 1000$, or 1000 grasp plans $\Gamma$. 
...
In future work, we plan to scale our method to 3D objects. However, this could substantially increase the number of candidate grasp plans.
To handle a much larger amount of arms, it may be useful to have some measure of correlation between the arms.
For example, if two grasps contact an object at spatially close locations, then their measure of probability of force closure may be correlated.
Appropriate features that represent a grasp or object under uncertainty and the grasp quality metric are still open questions and will be an ongoing project for future work. 
