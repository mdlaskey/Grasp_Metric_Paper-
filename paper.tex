...
%Grasp quality evaluation under uncertainty is ...

In new robotic environments, uncertainty in pose and shape can arise from sensor noise, occlusions, and surface properties such as transparency and specularity.

We show that framing grasp quality evaluation as a multi-armed bandit problem provides a principled way to reason about evaluating multiple grasps on an uncertain shape quickly.

We demonstrate this method for planning grasps in the presence of shape uncertainty encoded by a Gaussian process implicit surface (GPIS).

We utilize successive elimination to solve a multi-armed bandit problem and show a speedup in sample complexity versus evaluating grasps using Monte-Carlo integration, a popular method.
...
The robot may need to plan grasps for these objects without any prior information other than its sensor data.

As seen in Fig. \ref{fig:noisy data}, it is unavoidable that sensors will have missing or noisy data due to surface properties, such as specularity and transparency.

Grasp planners based on an exact representation of shape may fail.

This motivates using a representation of shape uncertainty to select grasps, but most current methods for evaluating quality under these representations use a brute-force sampling of shapes to evaluate grasp quality, which can be slow.



%To plan grasps in a partially observed world, recent research has focused on incorporating uncertainty in the shape of the object \cite{kehoe2012estimating}, \cite{christopoulos2007handling}, \cite{zheng2005}.

Often when evaluating grasps in this manner, there are a number of grasps that can be quickly shown to be inferior.

When allocating resources to determine the best grasp, this information should be used in order to spend less time evaluating bad grasps.

Multi-armed bandits provide a principled way to reason about evaluating multiple grasps and discarding grasps that are likely to be inferior.
...
\begin{enumerate}
	...
	\item Framing the selection of a grasp as from a given set as a multi-arm bandit problem.

	\item Demonstrating empirically that successive elimination, a multi-armed bandit algorithm, reduces the sampling complexity to find a grasp with high expected Ferrari-Canny quality from a set of candidate grasps.	
...
\section{Multi-Arm Bandit for Grasp Selection}
