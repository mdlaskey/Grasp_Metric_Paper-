...
We compare on the Brown Vision 2D Dataset, a dataset of 2D planar objects~\cite{brown, christopoulos2007handling}. We demonstrate performance on a local implementation. However, our methods are parallelizable by dividing the arms into $M$ subset,where $M$ is the number of available cores, and solving each MAB problem separate \cite{glazebrook2000index}. 
...
We propose modeling the problem as a Multi-Armed Bandit, which selects the next grasp to sample based on past observations ~\cite{barto1998reinforcement, lai1985asymptotically}. 
...
The measurement noise were assigned uniformly at random to the signed distance field.
